...
### Local APK Build Prerequisites

If building locally with `./gradlew :app:assembleDebug`, make sure an Android SDK is configured first.
Gradle expects either:

- `ANDROID_HOME` to point to a valid Android SDK directory, or
- a `local.properties` file in the repository root with `sdk.dir=/absolute/path/to/Android/Sdk`

Without one of these, Gradle fails during project configuration with `SDK location not found` before Java/Kotlin compilation starts.
