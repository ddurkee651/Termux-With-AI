...
public class TermuxTerminalExtraKeys extends TerminalExtraKeys {
    ...
    private void openRightDrawer() {
        TermuxActivity activity = mTermuxTerminalViewClient.getActivity();
        if (activity != null) activity.openRightDrawer();
    }

    private void showDrawersMenu(View anchor) {
        PopupMenu menu = new PopupMenu(mActivity, anchor);
        menu.getMenu().add(0, 1, 0, "Left drawer");
        menu.getMenu().add(0, 2, 1, "Right drawer");
        menu.setOnMenuItemClickListener((MenuItem item) -> {
            int id = item.getItemId();
            if (id == 1) toggleLeftDrawer();
            else if (id == 2) openRightDrawer();
            return true;
        });
        menu.show();
    }
    ...
    public void onTerminalExtraKeyButtonClick(View view, String key, boolean ctrlDown, boolean altDown, boolean shiftDown, boolean fnDown) {
        ...
        } else if ("DRAWERS".equals(key) || "DRAWER".equals(key)) {
            showDrawersMenu(view);
